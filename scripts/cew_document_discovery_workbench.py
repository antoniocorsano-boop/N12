...
function render(){q('status').innerHTML=`<b>${h(state.source_registration_state)}</b><br>${state.page_count} pagine · ${state.primitive_candidate_count} primitive · ${state.graphic_cluster_count} cluster<br>Structured: ${h(state.provider_states.structured_graphic.state)} · DINOv3: ${h(state.provider_states.visual_foundation.state)}<br>Etichette automatiche: no`;q('gate').className='gate '+(state.teaching_enabled?'ready':'blocked');q('gate').textContent=state.teaching_enabled?'SourceVersion + Page READY: training project-local consentito.':'Preview analizzabile, ma training bloccato fino alla registrazione SourceVersion/Page.';clusters();concepts();if(!clusterId&&state.clusters.length){clusterId=state.clusters[0].cluster_id;clusters();selected()}buttons()}
async function load(){state=await api(`/api/workbench/document-discovery/session/${encodeURIComponent(session)}`);render()}async function boot(){const b=await api('/api/workbench/document-discovery/status');for(const s of b.governed_sources){const o=document.createElement('option');o.value=s.source_id;o.textContent=`${s.source_id} · ${s.ready_page_count} Page READY`;q('source').appendChild(o)}}
q('analyze').onclick=async()=>{try{const project=q('project').value.trim(),source=q('source').value;if(!project||!source)throw Error('Indica progetto e fonte.');const b=await api('/api/workbench/document-discovery/analyze-governed',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project_id:project,source_id:source})});session=b.session_id;clusterId=null;await load()}catch(e){q('message').textContent=e.message}};q('preview').onclick=async()=>{try{const project=q('project').value.trim(),f=q('file').files[0];if(!project||!f)throw Error('Indica progetto e PDF.');const r=await fetch(`/api/workbench/document-discovery/analyze-preview?project_id=${encodeURIComponent(project)}`,{method:'POST',headers:{'Content-Type':'application/pdf'},body:await f.arrayBuffer()});const b=await r.json();if(!r.ok)throw Error(b.reason||b.state);session=b.session_id;clusterId=null;await load()}catch(e){q('message').textContent=e.message}}
...
def build_router(source_workspace) -> APIRouter:
    ...
    def status():
        try:
            return _json({"state":"DOCUMENT_DISCOVERY_AVAILABLE","governed_sources":discovery.governed_sources(source_workspace),"preview_allowed":True,"preview_teaching_allowed":False,"provider_states":discovery.provider_states()})
    ...
    async def analyze_governed(request: Request):
        try:
            body=await request.json(); session=discovery.create_governed(source_workspace,body.get("source_id"),body.get("project_id")); return _json(discovery.status(session["session_id"]))
    ...
    async def analyze_preview(request: Request, project_id: str=""):
        try:
            if str(request.headers.get("content-type") or "").split(";",1)[0].lower()!="application/pdf": raise ValueError("DOCUMENT_DISCOVERY_APPLICATION_PDF_REQUIRED")
            session=discovery.create_preview(await request.body(),project_id); return _json(discovery.status(session["session_id"]))
    ...
    def session_status(session_id: str):
        try: return _json(discovery.status(session_id))
        except ValueError as exc: return _json({"state":"DOCUMENT_DISCOVERY_SESSION_REJECTED","reason":str(exc)},404)
    ...
    def page_image(session_id: str,page_index: int):
        try: return Response(discovery.render_page(session_id,page_index),media_type="image/jpeg",headers={"Cache-Control":"no-store","X-CEW-Authority":"READING_AID_ONLY"})
        except ValueError as exc: return _json({"state":"DOCUMENT_DISCOVERY_PAGE_REJECTED","reason":str(exc)},404)

    @router.post("/api/workbench/document-discovery/session/{session_id}/learn")
    async def learn(session_id: str,request: Request):
        try: return _json(discovery.teach(session_id,await request.json()))
        except ValueError as exc: return _json({"state":"DOCUMENT_DISCOVERY_LEARNING_REJECTED","reason":str(exc)},409)
        except Exception as exc: return _json({"state":"DOCUMENT_DISCOVERY_LEARNING_BLOCKED","reason":str(exc)},503)

    @router.post("/api/workbench/document-discovery/session/{session_id}/similar")
    async def similar(session_id: str,request: Request):
        try:
            body=await request.json(); return _json(discovery.find_similar(session_id,body.get("concept_id"),body.get("meaning"),int(body.get("limit",40))))
        except ValueError as exc: return _json({"state":"DOCUMENT_DISCOVERY_SIMILARITY_REJECTED","reason":str(exc)},409)
