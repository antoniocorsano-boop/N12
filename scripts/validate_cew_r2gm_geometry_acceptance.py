...
def main() -> int:
    ...
    for region_id, proposal in proposals.items():
        ...
        require(proposal["human_supported_bridge_count"] > 0, "synthetic all-supported prerequisite should contribute reviewed bridges")
    ...
    try:
        ...
        for region_id in sorted(proposals):
            proposal = r2gm.build_region_proposal(runtime_r2gi, region_id)
            receipt = r2gm_receipt(proposal)
            envelope = r2gm.audit_envelope(f"R2GM-RUNTIME-{region_id}", receipt, proposal)
