...
def main() -> int:
    ...
    if len(pt_face_nodes) != 104:
        raise ValueError(f"PT face-node count {len(pt_face_nodes)} != 104")
    ...
    if role_counts["BEAM_SUPPORT_FACE"] != 464:
        raise ValueError(f"beam-face count {role_counts['BEAM_SUPPORT_FACE']} != 464")
    if len(rows) != 629:
        raise ValueError(f"global node count {len(rows)} != 629")
    expected_level_nodes = {"G1": 142, "G2": 130, "G3": 130, "G4": 130, "G5": 97}
    ...
    audit("TOTAL_3D_NODE_ROWS", 629, len(rows))
    audit("SUPPORT_CORE_ROWS", 165, role_counts["SUPPORT_CORE"])
    audit("G1_REUSED_PT_FACE_ROWS", 104, len(pt_face_nodes))
    ...
    audit("UNIQUE_NODE_IDS", 629, len(set(ids)))
