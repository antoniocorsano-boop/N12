...
EXPECTED_GROUPS={f'F1A-G0{i}' for i in range(1,8)}; EXPECTED_PATCH_IDS={f'M1F-REINF-P{i:03d}' for i in range(1,22)}
def read_csv(p):
    with p.open('r',encoding='utf-8-sig',newline='') as f:return list(csv.DictReader(f))
def apply_patch(base,patch,errors):
    rows=[dict(r) for r in base]; by={r['row_id'].strip():r for r in rows}
    if len(patch)!=21 or {r['patch_id'].strip() for r in patch}!=EXPECTED_PATCH_IDS: errors.append('HiRes correction/partition patch must contain exactly P001-P021'); return rows
    for p in patch:
...
def main():
    ...
    if len(reinf)!=60: errors.append(f'effective rows must be 60, got {len(reinf)}')
    ...
    if (complete,partial,pending)!=(6,1,0): errors.append(f'queue partition expected 6/1/0, got {(complete,partial,pending)}')
    open_rows=[r for r in reinf if r['binding_state'].strip().startswith('OPEN_')]
    if len(open_rows)!=1 or open_rows[0].get('row_id','').strip()!='F1A-G02-R04': errors.append(f'only G02-R04 may remain open, got {[r.get("row_id","") for r in open_rows]}')
    ...
    if len(g03_bent)!=7 or any(r['binding_state'].strip()!='GROUP_BOUND_WITH_22_PRIME_CORRECTION' for r in g03_bent): errors.append('G03 must contain exactly seven bent bars with 22-prime correction binding')
    ...
    gb={r['check_id'].strip():r for r in gate}; expected={'M1F-REINF-001':'7','M1F-REINF-002':'7','M1F-REINF-003':'60','M1F-REINF-004':'6','M1F-REINF-005':'1','M1F-REINF-006':'0','M1F-REINF-007':'1','M1F-REINF-008':'2','M1F-REINF-009':'1','M1F-REINF-010':'42','M1F-REINF-011':'1','M1F-REINF-012':'15','M1F-REINF-013':'4','M1F-REINF-014':'3','M1F-REINF-015':'2','M1F-REINF-016':'1','M1F-REINF-017':'4','M1F-REINF-018':'7','M1F-REINF-GATE':'PASS_GROUP_COVERAGE_WITH_TRANSCRIPTION_WATCHES'}
