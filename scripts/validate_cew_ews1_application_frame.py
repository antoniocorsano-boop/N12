...
def main() -> int:
    ...
    require(ews1["desktop"]["application_frame"] == "VIEWPORT_BOUND", "frame must be viewport bound")
    require(ews1["desktop"]["workspace_page_growth"] == "FORBIDDEN", "page growth guard missing")
    require(ews1["desktop"]["context_rail_overflow"] == "INDEPENDENT_VERTICAL_SCROLL", "rail scroll guard missing")
    require(ews1["oa_unregistered_source_position"]["topology"] == "SOURCE_PRIMARY_WITH_CONTEXT_RAIL", "OA topology drift")
    require(ews1["oa_unregistered_source_position"]["source_canvas_min_target_percent"] >= 65, "canvas target weakened")
    ...
    require("clamp(320px,30vw,440px)" in html, "governed rail width missing")
    ...
    print("OA_UNREGISTERED_TOPOLOGY = SOURCE_PRIMARY_WITH_CONTEXT_RAIL")
