...
document.body.dataset.cewLayoutPhaseGate='v2';
...
function describe(){
  ...
  const cx=us.map(u=>Number(u.x||0)+Number(u.w||0)/2),cy=us.map(u=>Number(u.y||0)+Number(u.h||0)/2);
  const spanX=Math.max(...cx)-Math.min(...cx),spanY=Math.max(...cy)-Math.min(...cy);
...
function showStructure(){
  ...
  for(const b of document.querySelectorAll('#cew-inspector-tabs button')){
    const on=b.id==='cew-layout-structure-tab';
...
function ensurePanel(){
  ...
  if(!tab){tab=document.createElement('button');tab.id='cew-layout-structure-tab';tab.type='button';tab.textContent='Struttura';tab.setAttribute('role','tab');tab.addEventListener('click',showStructure);tabs.insertBefore(tab,tabs.firstChild)}
  let p=ce('cew-layout-phase-panel');
  if(!p){
    ...
    ce('cew-layout-alternative').addEventListener('click',alternative);
...
function updatePanel(){
  const d=describe();
  setText('cew-layout-proposal',d.label);setText('cew-layout-reading',d.reading);setText('cew-layout-repeat',d.repeat);
  ...
  let local='Prima conferma come va letta la tavola.';
  if(gate.confirmed)local=gate.activeUnit?`Unità ${gate.activeUnit}: ${gate.localCandidates.length} frammenti grafici locali rilevati.`:'Seleziona una unità; CEW analizzerà automaticamente solo quella zona.';
  setText('cew-local-summary',local);
  const block=ce('cew-local-block');const blocked=gate.confirmed&&gate.activeUnit&&!gate.semanticReady;
  if(block){block.hidden=!blocked;if(blocked)block.textContent='La semantica resta bloccata: i vecchi cluster globali non vengono riutilizzati. Serve un candidato backend realmente locale.'}
...
function syncSemantics(){
  const ready=!!gate.semanticReady;document.body.dataset.cewSemanticReady=ready?'true':'false';
  const tab=ce('cew-decision-tab');if(tab){tab.hidden=!ready;tab.disabled=!ready;tab.title=ready?'Decisione semantica sul candidato locale':'Disponibile solo dopo struttura confermata e candidato locale collegato'}
  for(const id of ['pos','neg','amb','similar']){const b=ce(id);if(b&&!ready)b.disabled=true}
}

function confirm(){
  if(!units().length){feedback('Nessuna unità di lettura disponibile da confermare.');return}
  gate.confirmed=true;gate.phase=LAYOUT_CONFIRMED;writeConfirmed(true);document.body.dataset.cewLayoutPhase='confirmed';
  feedback('Struttura confermata. Seleziona una unità di lettura per avviare l’analisi locale.');updatePanel();showStructure();syncSemantics();
}
function reset(){
  gate.confirmed=false;gate.phase=LAYOUT_UNCONFIRMED;gate.activeUnit=null;gate.localCandidates=[];gate.semanticReady=false;writeConfirmed(false);document.body.dataset.cewLayoutPhase='unconfirmed';clearLocal();feedback('');updatePanel();showStructure();syncSemantics();
}
function alternative(){
  window.CEWLayoutLearning?.cycle?.();gate.activeUnit=null;gate.localCandidates=[];gate.semanticReady=false;clearLocal();feedback('Alternativa di impaginazione mostrata; verificala prima della conferma.');setTimeout(()=>{updatePanel();showStructure();syncSemantics()},50);
}

function imageMatrix(maxSide=760){
  const img=ce('page');if(!img||img.hidden||!img.naturalWidth||!img.naturalHeight)return null;
  const s=Math.min(1,maxSide/Math.max(img.naturalWidth,img.naturalHeight)),w=Math.max(80,Math.round(img.naturalWidth*s)),h=Math.max(80,Math.round(img.naturalHeight*s));
  const c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{willReadFrequently:true});if(!x)return null;x.drawImage(img,0,0,w,h);
  const d=x.getImageData(0,0,w,h).data,l=new Uint8Array(w*h);let sum=0;for(let i=0,p=0;i<d.length;i+=4,p++){const v=Math.round(.2126*d[i]+.7152*d[i+1]+.0722*d[i+2]);l[p]=v;sum+=v}
  const t=clamp(sum/Math.max(1,l.length)-24,115,226),ink=new Uint8Array(w*h);for(let i=0;i<l.length;i++)ink[i]=l[i]<t?1:0;return {w,h,ink};
}
function localFragments(u){
  const m=imageMatrix();if(!m)return [];const x0=clamp(Math.floor(u.x*m.w),0,m.w-1),x1=clamp(Math.ceil((u.x+u.w)*m.w),1,m.w),y0=clamp(Math.floor(u.y*m.h),0,m.h-1),y1=clamp(Math.ceil((u.y+u.h)*m.h),1,m.h),W=x1-x0,H=y1-y0;if(W<6||H<6)return [];
  const a=new Uint8Array(W*H);for(let y=0;y<H;y++)for(let x=0;x<W;x++)a[y*W+x]=m.ink[(y+y0)*m.w+x+x0];
  const dil=new Uint8Array(a.length);for(let y=0;y<H;y++)for(let x=0;x<W;x++)if(a[y*W+x])for(let yy=Math.max(0,y-1);yy<=Math.min(H-1,y+1);yy++)for(let xx=Math.max(0,x-1);xx<=Math.min(W-1,x+1);xx++)dil[yy*W+xx]=1;
  const seen=new Uint8Array(dil.length),out=[];for(let p=0;p<dil.length;p++){if(!dil[p]||seen[p])continue;const q=[p];seen[p]=1;let k=0,minx=W,miny=H,maxx=0,maxy=0,n=0;while(k<q.length){const z=q[k++],x=z%W,y=Math.floor(z/W);n++;minx=Math.min(minx,x);maxx=Math.max(maxx,x);miny=Math.min(miny,y);maxy=Math.max(maxy,y);for(let yy=Math.max(0,y-1);yy<=Math.min(H-1,y+1);yy++)for(let xx=Math.max(0,x-1);xx<=Math.min(W-1,x+1);xx++){const j=yy*W+xx;if(dil[j]&&!seen[j]){seen[j]=1;q.push(j)}}}const bw=maxx-minx+1,bh=maxy-miny+1,rel=bw*bh/(W*H);if(n>=8&&bw>=3&&bh>=2&&rel<=.72)out.push({x:(x0+minx)/m.w,y:(y0+miny)/m.h,w:bw/m.w,h:bh/m.h,pixels:n})}
  if(!out.length)return [{id:'LF-1',x:u.x,y:u.y,w:u.w,h:u.h,pixels:0,fallback:true}];return out.sort((a,b)=>b.pixels-a.pixels).slice(0,36).map((r,i)=>({...r,id:`LF-${i+1}`}));
}
...
function selectUnit(id){
  if(!gate.confirmed)return;const u=units().find(x=>x.id===id);if(!u)return;gate.activeUnit=id;gate.localCandidates=localFragments(u);renderLocal();const c=backendCandidateFor(u);gate.semanticReady=!!c;
  if(c){if(typeof clusterId!=='undefined')clusterId=c.cluster_id;if(typeof clusters==='function')clusters();if(typeof selected==='function')selected()}
  feedback(`Unità ${id} analizzata localmente.`);updatePanel();showStructure();syncSemantics();
}
function syncKey(){
...
function sync(){
  if(gate.syncing)return;gate.syncing=true;requestAnimationFrame(()=>{gate.syncing=false;syncKey();ensurePanel();updatePanel();syncSemantics()});
}
...
const obs=new MutationObserver(records=>{for(const r of records){if(r.target?.closest?.('#cew-layout-phase-panel'))continue;sync();break}});obs.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden','class','style']});
