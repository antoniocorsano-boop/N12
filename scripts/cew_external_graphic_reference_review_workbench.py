...
- materialize a non-promoting EXTERNAL_REFERENCE pack candidate only after the
  complete review queue has been decided.
...
def _validated_runtime_decisions() -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    acquisition, queue = _governed()
    ...
    decisions.sort(key=lambda row: str(row["review_item_id"]))
    return acquisition, queue, decisions


def decisions_document() -> dict[str, Any]:
    acquisition, queue, decisions = _validated_runtime_decisions()
    del acquisition
    return {
        "schema": DECISIONS_SCHEMA,
        "status": "HUMAN_REVIEW_COMPLETE" if len(decisions) == len(queue.get("review_items") or []) else "HUMAN_REVIEW_IN_PROGRESS",
...
def review_status() -> dict[str, Any]:
    acquisition, queue, decisions = _validated_runtime_decisions()
    del acquisition
    by_item = {str(row["review_item_id"]): row for row in decisions}
    items: list[dict[str, Any]] = []
    summary = {"UNREVIEWED": 0, "ACCEPT_REFERENCE_EVIDENCE": 0, "REJECT_REFERENCE_EVIDENCE": 0, "DEFER": 0}
    for item in queue.get("review_items") or []:
        decision = by_item.get(str(item["review_item_id"]))
...
def persist_review_receipt(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    _acquisition, _queue, existing = _validated_runtime_decisions()
    if any(str(row["review_item_id"]) == str(receipt["review_item_id"]) for row in existing):
        raise ValueError("REFERENCE_REVIEW_ITEM_ALREADY_DECIDED")
