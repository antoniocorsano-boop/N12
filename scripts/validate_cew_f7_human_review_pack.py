...
def main():
    ...
    if m.get('ui_language')!='it' or m.get('guided_review') is not True:raise AssertionError('guided Italian review surface missing')
    ...
    guided=('title_it','question_it','where_it','look_for_it','dont_it','confirmed_it','fallback_it')
    for e in m['entries']:
        if any(not e.get(k,'').strip() for k in guided):raise AssertionError(f'guided instruction missing: {e["task_id"]}')
    ...
    for token in ('CONFIRMED','direct_primary_evidence_observed','authority_acknowledgement','target_id','Esporta la decisione in JSON','Cosa devi controllare','Dove guardare','Cosa cercare','Non fare questo'):
        if token not in js:raise AssertionError(f'human decision control/guidance missing: {token}')
    if re.search(r'canonical_write[^\n]*true',json.dumps(m),flags=re.I):raise AssertionError('review pack gained canonical write')
    print('HUMAN_DECISION_INTAKE_PACK_PASS');print('TASKS=4/4');print('UI_LANGUAGE=it');print('GUIDED_REVIEW=YES');print('PREFILLED_OUTCOMES=0');print('PREFILLED_HUMAN_OBSERVATIONS=0');print('PREFILLED_DIRECT_PRIMARY_CLAIMS=0');print('PREFILLED_PROMOTION_TARGETS=0');print('PRIMARY_SOURCE_VIEWER=EMBEDDED');print('CANONICAL_WRITE=FORBIDDEN');print('M0G_REOPEN=FORBIDDEN');return 0
