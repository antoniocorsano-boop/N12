...
def validate() -> tuple[bool, list[str]]:
    errors: list[str] = []
    for path in [QUEUE_PATH, POLICY_PATH, CAPABILITY_PATH, CONTRACT_PATH, CHECKPOINT_PATH]:
        if not path.is_file():
...
def build_plan(queue: dict[str, Any], policy: dict[str, Any], registry: dict[str, Any], max_items: int | None = None) -> list[dict[str, Any]]:
    ...
    # A SERIAL item is an exclusive tranche. If the highest-priority eligible work is
    # SERIAL, nothing else is scheduled beside it.
...
def cmd_status(_: argparse.Namespace) -> int:
    ...
    plan = build_plan(queue, policy, registry)
    print(json.dumps({
        "status": "PASS",
        "queue_status": queue.get("status"),
        "canonical_boundary": queue.get("canonical_boundary"),
        "counts": counts,
        "eligible_count": len(eligible_items(queue)),
        "next_batch": plan,
...
def cmd_plan(args: argparse.Namespace) -> int:
    ...
    if not ok:
        ...
        return 2
    queue, policy, registry, _, _ = load_state()
    plan = build_plan(queue, policy, registry, args.max_items)
