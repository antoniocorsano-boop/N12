...
def _phase_status(raw: str) -> str:
    value = (raw or "").upper()
    if "B1_SOURCE_EVIDENCE_IN_PROGRESS" in value:
        return "Fonti ed evidenze in integrazione"
    if "PROJECT_HOME_V2" in value:
        return "Disponibile nel Project Home"
...
def _nav_item(item: dict) -> str:
    label = esc(item.get("label"))
    href = item.get("href")
    if href:
        return f"<a class='nav-chip nav-active' href='{esc(href)}'>{label}</a>"
    return f"<span class='nav-chip'>{label}</span>"
...
def build_project_home(state: dict, issues: dict, tasks: list[dict], terminology: dict, lifecycle: dict) -> str:
    ...
    nav = "".join(_nav_item(item) for item in terminology.get("navigation", []))
...
<section><h2>Accessi di lavoro</h2><div class="panel"><div class="nav-chips">{nav}</div><p class="muted">Fonti ed Evidenze sono operative nel work package B1. Le altre aree diventano operative progressivamente; la disponibilità tecnica non equivale alla validazione professionale del workflow.</p></div></section>
