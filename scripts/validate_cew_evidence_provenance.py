...
def main() -> int:
    ...
    for asset in assets:
        ...
        if src not in sources or page_id not in pages:
            raise AssertionError(f"derived asset parent missing: {asset['derived_asset_id']}")
    ...
    for row in regions:
        ...
        if src not in sources or page_id not in pages:
            raise AssertionError(f"region parent missing: {row['evidence_region_id']}")
        ...
        if row["readiness_state"].strip() == "READY":
            ...
            if row["coordinate_space"].strip() != "NORMALIZED_0_1" or row["geometry_type"].strip() != "BBOX":
                raise AssertionError(f"unexpected READY region geometry: {row['evidence_region_id']}")
