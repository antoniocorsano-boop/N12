...
def main() -> int:
    ...
    if len(effective_beams) != 49:
        fail(errors, f"effective physical beam count must be 49, got {len(effective_beams)}")
    ...
    if len(nodes) != 98:
        fail(errors, f"analytical node row count must be 98, got {len(nodes)}")
    ...
    if len(refs) != 98:
        fail(errors, f"Master must contain 98 analytical node references, got {len(refs)}")
    if len(set(refs)) != 98:
    ...
    expected_extended_counts = {"18": 2, "23": 2, "30": 3}
