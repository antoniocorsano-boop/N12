...
def _harden_template(template: str) -> str:
    replacements = {
        ...
        "if(t.task_id==='UX-DOC-03'){const macro=m.source_scale_states.some(v=>v.startsWith('MACRO'));const finalMicro=(m.source_scale_states[m.source_scale_states.length-1]||'').startsWith('MICRO');return macro&&finalMicro&&last.startsWith('/evidence/review?task=ERW-N12-001')?'SUCCESS':'FALSE_SUCCESS'}":
            "if(t.task_id==='UX-DOC-03'){const macro=m.source_scale_states.some(v=>v.startsWith('MACRO'));const finalMicro=(m.source_scale_states[m.source_scale_states.length-1]||'').startsWith('MICRO');const zoomed=m.viewer_states.some(v=>v.includes('Zoom usato'));const panned=m.viewer_states.some(v=>v.includes('Pan usato'));return zoomed&&panned&&macro&&finalMicro&&last.startsWith('/evidence/review?task=ERW-N12-001')?'SUCCESS':'FALSE_SUCCESS'}",
