...
<section id="oaG5Review" data-oag5-runtime="CEW_OAG5_RUNTIME_EXPLICIT_IDENTITY_REVIEW">
  ...
  <label>Decisione<select id="oaG5Decision"><option value="">— scegli esplicitamente —</option><option value="ACCEPT_STRUCTURAL_IDENTITY">ACCETTA IDENTITÀ</option><option value="REJECT_STRUCTURAL_IDENTITY">RIFIUTA IDENTITÀ</option><option value="DEFER_NEEDS_RELATIONSHIP_EVIDENCE">RINVIA — SERVE ALTRA EVIDENZA DI RELAZIONE</option></select></label>
...
function initOAG5(){const oa5=document.getElementById('oaStructuralResolver'),panel=document.getElementById('oaPanel');if(!panel||document.getElementById('oaG5Review'))return;(oa5||panel).insertAdjacentHTML('afterend',OAG5_SECTION);document.getElementById('oaG5Load').onclick=loadCandidate;document.getElementById('oaG5Save').onclick=saveDecision;window.cewOAG5Refresh=loadCandidate;loadCandidate()}
