...
def main() -> None:
    ...
    checks = [
        ("TOTAL_RIGID_LINK_ROWS", 458, len(out_rows), "One link for every beam-support face node."),
        ("G1_RIGID_LINK_ROWS", 98, by_level["G1"], "Reused PT face incidences."),
        ...
        ("UNIQUE_LINK_IDS", 458, unique_links, "No duplicate rigid links."),
        ("UNIQUE_FACE_NODE_TARGETS", 458, unique_faces, "Each face incidence is linked exactly once."),
        ("ORPHAN_CORE_NODES", 0, orphan_core, "Every face node must find the core of the same support and level."),
        ("SAME_SUPPORT_LINKS", 458, same_support, "Rigid offset never crosses physical supports."),
        ("SAME_LEVEL_LINKS", 458, same_level, "Rigid offset never crosses storeys."),
