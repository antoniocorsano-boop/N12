...
def augment(rendered: str, task: str) -> str:
    ...
    The runtime consumes only explicit OA-4 family-candidate decisions and explicit
    relationship evidence entered by the human. It cannot accept structural identity.
    """
...
<section id="oaStructuralResolver" data-oa5-runtime="CEW_OA5_RUNTIME_STRUCTURAL_RESOLVER">
  ...
  <div class="authority-note">Il risultato resta un candidato. L'accettazione dell'identità è un gate umano separato OA-G5.</div>
</section>'''
...
function admissibleDecisions(){const r=latestOA4();return (r?.candidate_decisions||[]).filter(d=>['CONFIRM_AS_FAMILY_CANDIDATE','MOVE_TO_OTHER_FAMILY'].includes(d.decision)&&d.family_membership_proposal_created===true)}
function loadCandidates(){const sel=document.getElementById('oa5Candidate'),rows=admissibleDecisions();sel.innerHTML=rows.map((d,i)=>'<option value="'+i+'">'+d.candidate_object_id+' · '+d.proposed_family_id+'</option>').join('');if(!rows.length)document.getElementById('oa5Result').innerHTML='<div class="oa5-block">Nessun candidato famiglia confermato disponibile. Completa prima OA-4.</div>'}
