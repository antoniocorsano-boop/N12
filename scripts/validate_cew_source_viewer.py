...
def main() -> int:
    ...
    required_files = [
        "index.html", "app.js", "styles.css", "viewer_manifest.json",
        "vendor/openseadragon/openseadragon.min.js",
        "tiles/TAV-05A.dzi", "tiles/TAV-06A.dzi",
    ]
    ...
    for token in ("openseadragon.min.js", "task-select", "viewer"):
        if token not in html:
            raise AssertionError(f"viewer HTML missing token: {token}")
    for token in ("URLSearchParams", "region", "task", "fitBounds", "showNavigator"):
        if token not in js:
            raise AssertionError(f"viewer deep-link/navigation behavior missing: {token}")
