...
<body><header><strong>CEW — Viewer della fonte primaria</strong><span id=\"task-label\"></span><div id=\"authority\"></div><div id=\"geometry\"></div></header><main><section><div id=\"viewer\"></div></section><aside><label>Task di risoluzione</label><select id=\"task-select\"></select><div><button id=\"fit\">Centra evidenza</button><button id=\"home\">Intera tavola</button></div><p><button id=\"original-mode\" aria-pressed=\"true\">Originale</button> <button id=\"enhanced-mode\" disabled>Migliorata — non disponibile</button></p><dl id=\"details\"></dl></aside></main><script src=\"app.js\"></script></body></html>"""
CSS = """html,body{margin:0;height:100%;font-family:system-ui,sans-serif}body{display:flex;flex-direction:column}header{padding:.6rem 1rem;border-bottom:1px solid #ccc;font-size:.8rem}header div{margin-top:.2rem}main{display:grid;grid-template-columns:minmax(0,1fr) 360px;min-height:0;flex:1}section{padding:.5rem;min-height:0}#viewer{height:100%;background:#202328}aside{padding:1rem;border-left:1px solid #ccc;overflow:auto}select{width:100%;margin:.4rem 0 1rem}.evidence-box{border:2px solid currentColor;box-sizing:border-box;pointer-events:none}dl{display:grid;grid-template-columns:110px 1fr;gap:.4rem}dd{margin:0;overflow-wrap:anywhere}@media(max-width:850px){main{grid-template-columns:1fr;grid-template-rows:65vh auto}}"""
