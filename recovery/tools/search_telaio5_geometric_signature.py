...
def solve(nodes,target,tol):
    ...
    def dfs(path,lens,angs,k):
        if k==8:
            ...
            # angular penalty rewards return to the baseline after each oblique transition
            base=angs[0]
        ...
        for b,ll in neigh[k][a]:
            ...
            if k>=1:
                ...
                if k==1:  # C2 = S'-T: first oblique transition
                    if not (OBLIQUE_MIN <= abs(dev) <= OBLIQUE_MAX): continue
                elif k==2:  # C3 = T-U: return near baseline
                    if abs(dev)>PARALLEL_TOL: continue
                elif k==3:  # C4 = U-V: second oblique, opposite side of baseline
                    d2=signed_turn(base,cand_angs[1])
                    if not (OBLIQUE_MIN <= abs(dev) <= OBLIQUE_MAX): continue
                    if d2*dev>=0: continue
                else:  # C5-C8 = V-Z-A'-B'-C': long near-collinear run
                    if abs(dev)>PARALLEL_TOL: continue
