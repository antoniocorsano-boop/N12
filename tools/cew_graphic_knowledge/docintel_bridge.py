...
def import_project_labels(docintel_db: Path, fabric_db: Path, project_id: str) -> dict[str, Any]:
    ...
    The bridge copies semantic training evidence, never source files and never canonical
    engineering values. Stable GCFP/source SHA lineage is mandatory.
    """
    ...
    try:
        ...
        rows = source.execute(
            """SELECT candidate_fingerprint,source_sha256,meaning,verdict,context_json,reviewer,created_at
...
def resolve_for_project(fabric_db: Path, project_id: str, context: dict[str, Any], limit: int = 5) -> dict[str, Any]:
    """Return transferable candidates; project human review remains authoritative."""
    result = gkf.resolve(fabric_db, project_id=project_id, context=context, limit=limit)
