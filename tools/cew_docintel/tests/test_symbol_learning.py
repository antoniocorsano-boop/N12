...
class SymbolLearningTests(unittest.TestCase):
    ...
    def test_label_dataset(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.run(CLI,'--db',str(db),'init')
            sv=json.loads(self.run(CLI,'--db',str(db),'ingest',str(src),'--source-id','S'))['source_version_id']
            obs=self.run(CLI,'--db',str(db),'observe','--source-version-id',sv,'--kind','symbol','--bbox','10,20,30,40','--value','rect','--confidence','0.9','--detector','test')
            self.run(LEARN,'--db',str(db),'label',obs,'--meaning','COLUMN_PLAN_MARKER','--verdict','POSITIVE','--reviewer','human','--context','{"drawing_type":"carpenteria"}')
            s=json.loads(self.run(LEARN,'--db',str(db),'stats'))
            self.assertEqual(s[0]['meaning'],'COLUMN_PLAN_MARKER'); self.assertEqual(s[0]['n'],1)
            ex=json.loads(self.run(LEARN,'--db',str(db),'examples','--meaning','COLUMN_PLAN_MARKER'))
