...
begin
  ...
  loop
    ...
    if v_receipt->'base_proposal_decision_id' is not null
       and v_receipt->'base_proposal_decision_id' <> 'null'::jsonb
       and nullif(trim(v_receipt->>'base_proposal_decision_id'), '') is null then
      raise exception 'OAR_REGION_BASE_PROPOSAL_DECISION_ID_INVALID' using errcode='23514';
    end if;

    if v_action not in ('PROPOSE_GEOMETRY','CONFIRM_GEOMETRY')
       or v_receipt->>'engineering_authority_effect' is distinct from 'NONE'
       or v_receipt->'canonical_write_authorized' is distinct from 'false'::jsonb
       or v_receipt->'structural_identity_authorized' is distinct from 'false'::jsonb
       or v_receipt->'oar_human_confirmation' is distinct from 'false'::jsonb
       or (v_action='PROPOSE_GEOMETRY' and v_receipt->>'authority' is distinct from 'WORKING_GEOMETRY_ONLY')
       or (v_action='CONFIRM_GEOMETRY' and v_receipt->>'authority' is distinct from 'HUMAN_EVIDENCE_LOCALIZATION_ONLY') then
      raise exception 'OAR_REGION_GOVERNED_AUTHORITY_MISMATCH' using errcode='23514';
    end if;
